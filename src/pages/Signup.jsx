...
import { useSignupMutation } from "../slices/auth/authApiSlice";
...
function Signup() {
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await signup({ type: "signup", user: formData });
      ...
      {token ? (
        ...
        <div className="w-100 flex h-screen items-center justify-center bg-gray-50">
          <div className="mx-auto flex w-2/3 flex-col items-center rounded-lg border border-t-gray-100 bg-white px-8 py-8 shadow-lg md:w-[500px] md:py-16">
            <h1 className="md:text3xl mb-10 text-2xl font-bold">Sign Up</h1>
            ...
            <div className="my-4 text-gray-400">
              Already a user?
              <Link to="/login"><span className="pl-1">Sign in</span></Link>
            </div>
          </div>
        </div>
      )}
